...
import {
  createPromptSubmit,
  promptExecutionTargets,
  type FollowupDraft,
  type PromptExecutionTarget,
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const [execution, setExecution] = persisted(
    Persist.global("prompt-execution-target", ["prompt-execution-target.v1"]),
    createStore({
      target: "mimo" as PromptExecutionTarget,
    }),
  )

  const suggest = createMemo(() => !hasUserPrompt())
  const currentExecutionTarget = () => promptExecutionTargets.find((target) => target === execution.target) ?? "mimo"
  const executionLabel = (target: PromptExecutionTarget) =>
    language.t(`prompt.executionTarget.${target}` as Parameters<typeof language.t>[0])
  ...
  const targetModel = createMemo(() => local.model.forTarget(currentExecutionTarget()))
  const variants = createMemo(() => ["default", ...targetModel().variant.list()])
  ...
  const { abort, handleSubmit } = createPromptSubmit({
    ...
    onConfigureMimo: () => {
      if (providers.all().some((provider) => provider.id === "xiaomi")) {
        void import("./dialog-connect-provider").then((x) => {
          dialog.show(() => <x.DialogConnectProvider provider="xiaomi" initialMethod="api" />)
        })
        return
      }
      if (providers.all().some((provider) => provider.id === "mimo")) {
        void import("./dialog-connect-provider").then((x) => {
          dialog.show(() => <x.DialogConnectProvider provider="mimo" initialMethod="api" />)
        })
        return
      }
              ...
              <div class="flex items-center gap-1.5 min-w-0 flex-1 h-7">
                <Show when={!agentsLoading()}>
                  <Show when={store.mode !== "shell"}>
                    <div data-component="prompt-execution-target-control" style={{ animation: "fade-in 0.3s" }}>
                      <Tooltip placement="top" gutter={4} value={language.t("prompt.executionTarget.tooltip")}>
                        <Select
                          size="normal"
                          options={[...promptExecutionTargets]}
                          current={currentExecutionTarget()}
                          label={executionLabel}
                          onSelect={(value) => {
                            if (value) setExecution("target", value)
                            restoreFocus()
                          }}
                          class="max-w-[150px] text-text-base"
                          valueClass="truncate text-13-regular text-text-base"
                          triggerStyle={control()}
                          triggerProps={{ "data-action": "prompt-execution-target" }}
                          variant="ghost"
                        />
                      </Tooltip>
                    </div>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                void import("@/components/dialog-select-model-unpaid").then((x) => {
                                  dialog.show(() => <x.DialogSelectModelUnpaid model={targetModel()} />)
                                })
                              }}
                            >
                              <Show when={targetModel().current()?.provider?.id}>
                                <ProviderIcon
                                  id={targetModel().current()?.provider?.id ?? ""}
                                  class="size-4 shrink-0 opacity-40 group-hover:opacity-100 transition-opacity duration-150"
                                  style={{ "will-change": "opacity", transform: "translateZ(0)" }}
                                />
                              </Show>
                              <span class="truncate">
                                {targetModel().current()?.name ?? language.t("dialog.model.select.title")}
                              </span>
                    ...
                    <div data-component="prompt-variant-control" style={{ animation: "fade-in 0.3s" }}>
                      ...
                      >
                        <Select
                          size="normal"
                          options={variants()}
                          current={targetModel().variant.current() ?? "default"}
                          label={(x) => (x === "default" ? language.t("common.default") : x)}
                          onSelect={(value) => {
                            targetModel().variant.set(value === "default" ? undefined : value)
                            restoreFocus()
                          }}
                          class="capitalize max-w-[160px] text-text-base"
                          valueClass="truncate text-13-regular text-text-base"
