...
export function DialogConnectProvider(props: { provider: string; initialMethod?: "api" }) {
  ...
  const methods = createMemo(() => {
    const list = auth.latest ?? globalSync.data.provider_auth[props.provider] ?? fallback()
    if (props.provider !== "xiaomi") return list
    if (list.some((item) => item.type === "api")) return list
    return [
      ...list,
      {
        type: "api" as const,
  ...
  createEffect(() => {
    if (auto) return
    if (loading()) return
    if (props.initialMethod === "api") {
      const index = methods().findIndex((item) => item.type === "api")
      if (index === -1) return
      auto = true
