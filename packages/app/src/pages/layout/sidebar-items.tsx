...
const MimoProjectIcon = (props: { masked: boolean }): JSX.Element => (
  <div
    aria-hidden="true"
    class="size-full rounded border border-border-weak-base bg-background-base flex items-center justify-center"
    classList={{ "badge-mask": props.masked }}
    style={{ color: "color-mix(in srgb, var(--icon-base) 34%, var(--background-base))" }}
  >
    <svg viewBox="0 0 32 32" class="size-7" fill="none">
      <path
        d="M7.5 10.4 13.3 16 7.5 21.6"
        stroke="currentColor"
        stroke-width="2.7"
      ...
      />
      <path
        d="M18.2 10.8 24 14.3 18.8 18.3"
        stroke="currentColor"
        stroke-width="1.9"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <circle cx="18.2" cy="10.8" r="2.2" fill="currentColor" />
      <circle cx="24" cy="14.3" r="2.2" fill="currentColor" />
      <circle cx="18.8" cy="18.3" r="2.2" fill="currentColor" />
      <path d="M21.4 23.2H26.8" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" />
    </svg>
  </div>
)
...
export const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
  ...
  const projectIconSrc = createMemo(() => {
    if (props.project.icon?.override) return props.project.icon.override
    if (props.project.id === LEGACY_OPENCODE_PROJECT_ID) return
  ...
  return (
    <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
      <div class="size-full rounded overflow-clip">
        <Show
          when={props.project.id === LEGACY_OPENCODE_PROJECT_ID && !projectIconSrc()}
          fallback={
            <Avatar
              fallback={name()}
              src={projectIconSrc()}
