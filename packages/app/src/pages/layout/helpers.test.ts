...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("mimocode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
    expect(parseDeepLink("opencode://open-project?directory=/tmp/legacy")).toBe("/tmp/legacy")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("mimocode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("mimocode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("mimocode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("mimocode://open-project")).toBeUndefined()
    expect(parseDeepLink("mimocode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "mimocode://open-project?directory=/a",
      "mimocode://other?directory=/b",
      "mimocode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("mimocode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("mimocode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("mimocode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("mimocode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "mimocode://new-session?directory=/a",
      "mimocode://open-project?directory=/b",
      "mimocode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MIMOCODE__: {
        deepLinks: ["mimocode://open-project?directory=/a"],
      },
    } as unknown as Window & { __MIMOCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["mimocode://open-project?directory=/a"])
    expect(drainPendingDeepLinks(target)).toEqual([])
  })

  test("drains legacy global deep links", () => {
    const target = {
      __OPENCODE__: {
