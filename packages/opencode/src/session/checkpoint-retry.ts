...
export async function loadPriorDiscoveredTitles(
  ...
  const text = await Filesystem.readText(checkpointPath(sessionID)).catch(() => "")
...
export async function runValidatorsForCkpt(
  ...
  const checkpointContent = await Filesystem.readText(checkpointPath(sessionID)).catch(() => "")
  const memoryContent = await Filesystem.readText(memoryPath(injected.projectID)).catch(() => "")
