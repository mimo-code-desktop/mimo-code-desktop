...
export function apiUrls(model: Pick<Model, "options">) {
  const value = model.options["apiUrls"]
  if (!isRecord(value)) return {}
  return {
    openai: typeof value.openai === "string" && value.openai.trim() !== "" ? value.openai.trim() : undefined,
    anthropic:
      typeof value.anthropic === "string" && value.anthropic.trim() !== "" ? value.anthropic.trim() : undefined,
  }
}

export function apiUrlForSDK(model: Pick<Model, "api" | "options">) {
  const urls = apiUrls(model)
  if (model.api.npm.includes("@ai-sdk/anthropic")) return urls.anthropic
