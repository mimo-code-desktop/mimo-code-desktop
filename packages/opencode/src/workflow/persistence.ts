...
const writeScript = (runID: string, body: string) =>
  Effect.promise(async () => {
    await Filesystem.write(scriptPath(runID), body)
  })

const readScript = (runID: string) => Effect.promise(() => Filesystem.readText(scriptPath(runID)))
...
const loadJournal = (runID: string): Effect.Effect<JournalLoad> =>
  Effect.promise(async () => {
    if (!existsSync(journalPath(runID))) return { results: new Map(), pass: 1 }
    const text = readFileSync(journalPath(runID), "utf-8")
...
const clearJournal = (runID: string) =>
  Effect.promise(async () => {
    await Filesystem.write(journalPath(runID), "")
  })
