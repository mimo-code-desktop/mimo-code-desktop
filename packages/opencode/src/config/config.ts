...
function shouldReplacePath(path: string[]) {
  return path.length === 1 && path[0] === "mcp"
}

function patchJsonc(input: string, patch: unknown, path: string[] = []): string {
  if (shouldReplacePath(path)) {
...
function mergeWritableConfig(existing: Info, config: Info) {
  const merged = mergeDeep(writable(existing), writable(config))
  if (config.mcp !== undefined) merged.mcp = config.mcp
  return merged
}
  ...
  Effect.gen(function* () {
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info) {
      ...
      if (!file.endsWith(".jsonc")) {
        ...
        const merged = mergeWritableConfig(existing, config)
