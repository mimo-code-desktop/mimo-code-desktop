...
import { APP_NAME, APP_REPOSITORY_URL, UPDATER_ENABLED } from "./constants"
...
export function createMenu(deps: Deps) {
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    { role: "windowMenu" },
    {
      label: "Help",
      submenu: [
        { label: "MiMo Code Documentation", click: () => shell.openExternal(`${APP_REPOSITORY_URL}#readme`) },
        { label: "MiMo Code Repository", click: () => shell.openExternal(APP_REPOSITORY_URL) },
        { type: "separator" },
        {
          label: "Share Feedback",
          click: () => shell.openExternal(`${APP_REPOSITORY_URL}/issues/new?template=feature-request.yml`),
        },
        {
          label: "Report a Bug",
          click: () => shell.openExternal(`${APP_REPOSITORY_URL}/issues/new?template=bug-report.yml`),
        },
      ],
    },
  ]
