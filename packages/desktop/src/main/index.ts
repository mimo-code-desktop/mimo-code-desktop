...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) =>
      arg.startsWith(`${APP_PROTOCOL}://`) || arg.startsWith(`${APP_LEGACY_PROTOCOL}://`),
    )
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    ...
    app.setAsDefaultProtocolClient(APP_LEGACY_PROTOCOL)
    registerRendererProtocol()
    setDockIcon()
    setupAutoUpdater()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.MIMOCODE_PORT ?? process.env.OPENCODE_PORT
  if (fromEnv) {
