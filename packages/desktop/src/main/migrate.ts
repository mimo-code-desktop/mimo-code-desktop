...
// Legacy Tauri/OpenCode app identifiers used only as migration sources.
...
function tauriAppId() {
  return app.isPackaged ? LEGACY_TAURI_APP_IDS[CHANNEL] : LEGACY_TAURI_APP_IDS.dev
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `opencode.settings.dat` is special: it maps into the new MiMo settings store.
// All other .dat
// files keep their full filename as the electron-store name so legacy data still
// matches what the shared renderer passes via IPC (e.g. `"default.dat"`,
// `"opencode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opencode.settings.dat -> the MiMo electron settings store.
